"""
Block: load_and_export_raw_manga
Type: data_loader

Streams manga from MangaDex in date chunks and exports each chunk to Postgres
...
Pipeline variables (metadata.yaml):
    ...
    manga_since_date:           optional ISO date override — skips checkpoint lookup
...
from manga_tracker.utils.loaders.mangadex.manga import stream_raw_manga_by_chunk
...
_PIPELINE_NAME = "load_mangadex_manga"
...
def _export_chunk(rows: List[dict], config_profile: str, pipeline_uuid: str) -> None:
    """Upsert a batch of manga row dicts into raw.manga_responses."""
    ...
    print(f"[{pipeline_uuid}] Exported {len(df)} rows to raw.manga_responses.")
...
def load_and_export(**kwargs) -> pd.DataFrame:
    ...
    for chunk_end, chunk_records in stream_raw_manga_by_chunk(
    ...
    print(f"[{pipeline_uuid}] Done. Total manga exported: {total_exported}.")
